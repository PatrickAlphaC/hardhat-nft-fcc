// We are going to skimp a bit on these tests...

const { assert, expect } = require("chai")
const { network, deployments, ethers } = require("hardhat")
const { developmentChains } = require("../../helper-hardhat-config")
const { createImageUri, createTokenUri } = require("../../utils/createUris")
const fs = require("fs")

// const highTokenUri =
//     "data:application/json;base64,"
// const lowTokenUri =
//     "data:application/json;base64,"

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT Unit Tests", () => {
          let dynamicSvgNft, deployer, mockV3Aggregator
          const lowSvg = fs.readFileSync("./images/dynamicNft/frown.svg", { encoding: "utf8" })
          const highSvg = fs.readFileSync("./images/dynamicNft/happy.svg", { encoding: "utf8" })

          beforeEach(async () => {
              accounts = await ethers.getSigners()
              deployer = accounts[0]
              await deployments.fixture(["mocks", "dynamicsvg"])
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft", deployer)
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator", deployer)
          })

          describe("constructor", () => {
              it("sets the price feed correctly", async () => {
                  const priceFeed = await dynamicSvgNft.getPriceFeed()
                  assert.equal(priceFeed, mockV3Aggregator.address)
              })
              it("creates the low image URI correctly", async () => {
                  const _lowImageUri = createImageUri(lowSvg)
                  const lowImageUri = await dynamicSvgNft.svgToImageURI(lowSvg)
                  assert.equal(_lowImageUri, lowImageUri)
              })
              it("creates the high image URI correctly", async () => {
                  const _highImageUri = createImageUri(highSvg)
                  const highImageUri = await dynamicSvgNft.svgToImageURI(highSvg)
                  assert.equal(_highImageUri, highImageUri)
              })
              it("sets the name of the token correctly", async () => {
                  const name = await dynamicSvgNft.name()
                  assert.equal(name, "Dynamic SVG NFT")
              })
              it("sets the symbol of the token correctly", async () => {
                  const symbol = await dynamicSvgNft.symbol()
                  assert.equal(symbol, "DSN")
              })
          })

          describe("mintNft", () => {
              it("emits an event and creates the NFT", async function () {
                  const highValue = ethers.utils.parseEther("1") // 1 dollar per ether
                  await expect(dynamicSvgNft.mintNft(highValue))
                      .to.emit(dynamicSvgNft, "CreatedNFT")
                      .withArgs("1", highValue)
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(tokenCounter.toString(), "1")
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  const highImageUri = createImageUri(highSvg)
                  const highTokenUri = createTokenUri("Dynamic SVG NFT", highImageUri)
                  assert.equal(tokenURI, highTokenUri)
              })
              it("shifts the token uri to lower when the price doesn't surpass the highvalue", async function () {
                  const highValue = ethers.utils.parseEther("10")
                  const txResponse = await dynamicSvgNft.mintNft(highValue)
                  await txResponse.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  const highImageUri = createImageUri(highSvg)
                  const highTokenUri = createTokenUri("Dynamic SVG NFT", highImageUri)
                  assert.equal(tokenURI, highTokenUri)
                  const newAnswer = ethers.utils.parseEther("1")
                  const updateAggregatorAnswerTx = await mockV3Aggregator.updateAnswer(newAnswer) // sets the answer of the aggregator to be lower than `highValue`
                  await updateAggregatorAnswerTx.wait(1)
                  const updatedTokenURI = await dynamicSvgNft.tokenURI(0)
                  const lowImageUri = createImageUri(lowSvg)
                  const lowTokenUri = createTokenUri("Dynamic SVG NFT", lowImageUri)
                  assert.equal(updatedTokenURI, lowTokenUri)
              })
          })

          describe("getLowSVG", () => {
              it("reads the low image URI from storage", async () => {
                  const lowImageUri = await dynamicSvgNft.getLowSVG()
                  const _lowImageUri = createImageUri(lowSvg)
                  assert.equal(lowImageUri, _lowImageUri)
              })
          })

          describe("getHighSVG", () => {
              it("reads the high image URI from storage", async () => {
                  const highImageUri = await dynamicSvgNft.getHighSVG()
                  const _highImageUri = createImageUri(highSvg)
                  assert.equal(highImageUri, _highImageUri)
              })
          })

          // More coverage of course
          // Maybe some tokenURI oddities
      })
